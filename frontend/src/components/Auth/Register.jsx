  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        toast.success("Registration Successful", { theme });
        navigate("/dashboard");
      }
    } catch (err) {
      const message = err.response?.data?.message || "Email already registered";
  ...
  return (
    <div className={`register-container ${theme}`}>
      <form onSubmit={handleSubmit} className={`register-form ${theme}`}>
        ...
        <button className={`register-button ${theme}`}>Register</button>
